...
DELIMITER $$
CREATE PROCEDURE `SP_COMPETENCIAS_GENERAL_EVAL`(IN `IN_idCliente` VARCHAR(30), IN `IN_idServicio` VARCHAR(30), IN `IN_idProyecto` VARCHAR(30), IN `IN_tipoComparacion` VARCHAR(30), IN `IN_tipoCargo` VARCHAR(30), IN `IN_fechaIni` DATETIME, IN `IN_fechaFin` DATETIME, IN `IN_cicloEvaluacion` INT, OUT `out_codResp` CHAR(2), OUT `out_msjResp` VARCHAR(200))
BEGIN
